...
old_pauliarray_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "old_pauliarray")
)
if old_pauliarray_path not in sys.path:
    sys.path.insert(0, old_pauliarray_path)


from pauliarray.conversion.qiskit import operator_to_sparse_pauli as old_operator_to_sparse_pauli
from pauliarray.mapping.fermion import JordanWigner as old_JordanWigner
...
def sym_old_pauli():
    ...
    for mol_idx, mol_labels_position in enumerate(label_list):
        ...
        mapping = old_JordanWigner(second_q_op.num_spin_orbitals)
        ...
        qubit_hamiltonien = old_operator_to_sparse_pauli(qubit_hamiltonien).simplify()
...
def main():
    time_old = sym_old_pauli()
    # put list to json
    with open("./results/old_pa_times.json", "w") as f:
