...
std::vector<dpoint> xor_dpoint(const std::vector<dpoint>& a, const std::vector<dpoint>& b) {
    ...
    unsigned pos;
    if (i < a.size() && j < b.size()) pos = std::min(a[0].first, b[0].first);
    else if (i < a.size()) pos = a[0].first;
    else pos = b[0].first;
    ...
    while (i < a.size() || j < b.size()) {
        ...
        if (i < a.size()) next_begin = std::min(next_begin, a[i].first);
        if (j < b.size()) next_begin = std::min(next_begin, b[j].first);
